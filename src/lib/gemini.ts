...
import { buildAuthorizationRulesForPrompt } from "./authorizations";
...
async function processDocumentWithKey(
  ...
  let systemInstruction = "";
   if (mode === 'Expedientes') {
    ...
    CONTROL OBLIGATORIO DE REVISIÓN TÉCNICA VEHICULAR (RTV) — CRÍTICO:
    Si en el PIMyS, en la factura, en el detalle del comprobante o en la columna "Texto" del Libro Diario aparece cualquier mención a una Revisión Técnica Vehicular o a los trámites asociados (por ejemplo: "revisión técnica", "revision tecnica", "RTV", "verificación técnica vehicular", "VTV", "certificación de modificaciones", "homologación de modificaciones", "oblea de GNC", "renovación de oblea", "prueba hidráulica de cilindro", "cilindro GNC"), entonces el gasto DEBE estar imputado obligatoriamente al código 417 ("Revisión Técnica Vehicular, Certificación y Homologación de modificaciones adicionales para RTV. Renovación de Obleas de GNC y prueba hidráulica de cilindro para GNC").
    Si detectás una revisión técnica pero el pago fue imputado a CUALQUIER otro código (por ejemplo 412, u otro código de movilidades), la validación V3 DEBE calificarse obligatoriamente como 'fail' e indicar de forma explícita y visible: "ERROR DE IMPUTACIÓN: se trata de una Revisión Técnica Vehicular y debió imputarse al **código 417**, pero se utilizó el **código [NNN]**". No aceptes el código incorrecto aunque pertenezca al rubro de movilidades: la RTV tiene código propio y excluyente.
    V4: Aprobadores firmantes presentes en PIMyS con nombre y legajo.
    CONTROL DE AUTORIZACIONES ESPECIALES (CRÍTICO): además de los aprobadores habituales, ciertos códigos de gasto y ciertos agentes solicitantes exigen la firma o autorización de una persona determinada. Debés revisar SIEMPRE las dos listas siguientes y verificar cada autorización que corresponda al pago analizado. Si una autorización requerida no figura en el PIMyS ni en documentación adjunta (correo, nota, sello o firma), calificá V4 como 'fail' (o 'warning' si el documento no permite verificarlo con certeza) e indicá de forma explícita qué firma falta.
${buildAuthorizationRulesForPrompt()}
    ...
    - Ej V4: Detalla los aprobadores renglón por renglón exactamente así: "Aprobador 1: [Nombre]\nAprobador 2: **[Nombre / Si es correo pimysmovrafaela pon 'Movilidades Rafaela']**\nResponsable de FF: [Nombre]". Luego, por CADA autorización especial que corresponda (por código de gasto o por el agente Solicitante), añade una línea nueva con este formato exacto: "Control [Código NNN | Sector X]: Autorización de **[Nombre esperado]** - [Verificada/Falta]". Si el pago no dispara ninguna autorización especial, no agregues líneas de control.
    ...
    1. "expedienteNumero": Extrae el número de expediente de la sección "Referencia: Carátula del expediente EE-YYYY-00000XXXXX-APPSF-OD". Debe formarse con el año y el bloque numérico COMPLETO, CONSERVANDO TODOS LOS CEROS A LA IZQUIERDA tal como figuran en la carátula, con el formato "YYYY-00000XXXXX" (ej. de "EE-2026-00035146-APPSF-OD" debes extraer "2026-00035146", de "EE-2026-00008401-APPSF-OD" debes extraer "2026-00008401"). NUNCA recortes ni elimines los ceros iniciales del bloque numérico: el número debe quedar exactamente igual al de la carátula.
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          expedienteNumero: { type: Type.STRING, description: "Número de expediente de la sección 'Referencia', formato 'YYYY-00000XXXXX' conservando TODOS los ceros a la izquierda tal como figuran en la carátula, ej. '2026-00035146'." },
